...
**Description source icons (FEAT-03).** `frontend/src/components/DescriptionSourceIcon.tsx` is the single source. Catalog sources → gray `Database` span (display-only). `ai_generated` → purple `Sparkles` **button** that stops propagation and calls `onRegenerate()`. While regenerating, a `Loader2` with class `ds-spin` replaces it. `manual` / `null` render nothing. **Edit page:** `BookEditCard.handleRegenerate` calls `generateSummary()` then `onImmediateSave({description, description_source: 'ai_generated', needs_description_review: true})`. The edit-page icon source also falls back to `book.data_sources?.description` when `book.description_source` is null (for books saved before the CHANGES-19 follow-up fix). **Review step:** regeneration flows up to `PhotoWorkflowPage.handleRegenerateSummary`, which bumps `aiGenIdRef` and flips `aiSummary` back through `pending` → `success`/`failed`. `effectiveDescriptionSource` = `aiDescription ? 'ai_generated' : lookupResult.data_sources?.description ?? null`. **Save path (CHANGES-19 follow-up):** `ReviewStep.handleSave` now correctly passes the actual catalog source (e.g. `'google_books'`) to `description_source` on POST; prior code sent `null` for catalog books via a dead ternary, defeating the backend auto-derive and leaving catalog-sourced books without an icon on the edit page. `needs_description_review` now also honors the user's toggle regardless of whether an AI summary exists.
