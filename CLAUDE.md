...
**Post-iteration layout + editability fixes.** Several bugs surfaced after the initial CHANGES-20 merge and were fixed in the same branch: (1) Title and Author wrapped in block-level `.bc-title-row` / `.bc-author-row` divs so the inline-block `InlineField` spans no longer flow side-by-side. (2) Vertical spacing in the title block: `margin-top: 14px` on `.bc-title-row`, `margin-bottom: 12px` on `.bc-author-row`, `margin-bottom: 8px` on `.bc-field-full`, `margin-bottom: 20px` on `.bc-row-inline`. Edit-view content wrapper gained `1rem 1.25rem` padding. (3) Edit-view inner wrapper has `minHeight: 100%` so its L/R borders reach the footer's top border — no white gap below short content. (4) `InlineField` now has a `compact` prop used by Year / ISBN / Pages: input has `width: Nch` based on content length and `display: inline-block` so tapping edits in place instead of dropping to the next line. Default mode is `display: block` with `whiteSpace: pre-wrap` for the multiline description. (5) ISBN is now fully editable — `isbn` added to `BookCard.DraftFields` and `commitDraft()` patch, and added as `Optional[str]` to the FastAPI `BookUpdate` schema so `PATCH /books/{id}` accepts ISBN updates (backend DB has a unique index — collisions raise on save). (6) Description block no longer uses `.bc-field-full` flex wrapper — the `align-items: baseline` was collapsing multiline content. Editable path uses `InlineField` directly; non-editable path uses a block `<div>` with an italic "No description" placeholder when empty so missing content is visible instead of silently collapsing.
