...
**BookEditCard inline editing.** Anchored layout using the visualViewport pattern (mirrors `WorkflowWrapper`). Outer fixed container is `maxWidth: 1200; margin: 0 auto` (matches DashboardPage) with a 1px border + `radius.md` so the card reads as a contained unit on wide viewports. Header zone (`zoneBg`): pill-shaped Back button (chevron + "Back") + "Edit Book" title, calls `onBack` prop. Filmstrip flush below header. Scrollable content zone in the middle — fills the full card width so header/filmstrip/content/footer all align. Footer zone (`zoneBg`): "added {date}" left, Save button right. Same layout desktop and mobile. All text fields use `InlineField`. Pending edits accumulate in `DraftFields` and commit together on Save. ISBN is read-only. Condition is a 5-button segmented bar (`ConditionBar`) — saves immediately via `onImmediateSave`. Checkboxes (Review Metadata?, Review Photography?) live in a single bordered container, save immediately. Empty fields display as `—`. After a successful Save, `onSaved` fires so DashboardPage clears `editingBook` and returns to the table. The Listing generator is no longer triggered from inside the card — only from BookTable rows.

**`data_complete` override preservation on save.** `PATCH /books/{id}` auto-recomputes `data_complete` from the key fields whenever the caller does NOT include it in the payload (`api/app/routers/books.py`). Without intervention this clobbers a user's manual "review metadata" override the moment they hit Save. `BookEditCard.handleSaveChanges` therefore explicitly forwards `data_complete: book.data_complete` (and `needs_photo_review` for symmetry) on every save so the override sticks.
